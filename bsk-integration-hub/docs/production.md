...
## Что уже сделано для боевой готовности

- ✅ **meta/href** и payload’ы документов МС (заказ поставщику, счёт поставщика).
- ✅ **Сверка входящих вебхуков**: Б24 `application_token`, МС — секрет в query.
- ✅ **429/rate-limit, backoff/ретраи, пагинация, нормализация ошибок** МС.
- ✅ **Нормализация payload** робота Б24 (form/bracketed) + смарт-процессы
  (`entityTypeId`/`itemId`/`stageId`).
- ✅ **Идемпотентность** и **reference-mappings** (товары/контрагенты/склады/орг).
- ✅ Dry-run превью процесса `create_supplier_docs` (полный план без записи).

См. безопасную обкатку на песочнице — [sandbox.md](./sandbox.md).

## Остаётся проверить на песочнице (перед первыми записями)

1. Точные **коды UF-полей** Б24 для записи id документов МС
   (`BITRIX24_WRITEBACK_*`) и реальные имена событий смарт-процесса.
2. **Обязательные поля** документов МС в вашем аккаунте (организация, склад,
   договор, тип цен/НДС) — на их основе доуточнить payload.
3. Поведение **реальных лимитов** МС под нагрузкой.

До подтверждения этих пунктов держите `DRY_RUN=true` (этап А — «реальные чтения
+ dry-run») и проверяйте снимки/предпросмотр. См. `ROADMAP.md`.
