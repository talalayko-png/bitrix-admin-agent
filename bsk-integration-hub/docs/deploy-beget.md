# Первый деплой на VPS Beget (встроенный терминал) — hub.bsk-grupp.ru
...
проверить `https://hub.bsk-grupp.ru/health`. **Без боевых секретов Б24/МС, без
...
## 3. Проверка DNS для hub.bsk-grupp.ru

```bash
getent hosts hub.bsk-grupp.ru || true
# или, если установлен dig:
dig +short hub.bsk-grupp.ru A
```

IP в ответе должен совпадать с внешним IP из шага 2. Если нет — заведите в DNS
домена `bsk-grupp.ru` **A-запись** `hub` → внешний IP сервера и подождите
...
sed -i "s|^DOMAIN=.*|DOMAIN=hub.bsk-grupp.ru|" .env
sed -i "s|^ACME_EMAIL=.*|ACME_EMAIL=admin@bsk-grupp.ru|" .env
sed -i "s|^CORS_ORIGINS=.*|CORS_ORIGINS=https://hub.bsk-grupp.ru|" .env
...
curl -s https://hub.bsk-grupp.ru/health; echo
```

Ожидаемо: `{"status":"ok","dry_run":true,"real_api_enabled":false,"queue_backend":"redis",...}`.

Откройте в браузере `https://hub.bsk-grupp.ru/health` и `https://hub.bsk-grupp.ru/docs`.
...
1. **DNS** (шаг 3): `getent hosts hub.bsk-grupp.ru` → внешний IP сервера. Если не
...
2. **Порт 80 доступен из интернета** (Let's Encrypt проверяет по HTTP-01 на :80):
   ...
   curl -s http://hub.bsk-grupp.ru/health; echo   # должен ответить
   ```
